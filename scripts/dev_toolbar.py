"""
Development mode: Run toolbar directly with auto-reload on file changes.
This is MUCH faster than rebuilding the EXE every time.

Usage:
    python dev_toolbar.py
"""
...
class ReloadHandler(FileSystemEventHandler):
    ...
    def should_reload(self, file_path):
        ...
        if path.name == 'dev_toolbar.py':
            return False
        
        return True
    
    def start_process(self):
        """Start the toolbar process."""
...
def main():
    ...
    toolbar_script = script_dir / 'src' / 'toolbar.py'
    
    if not toolbar_script.exists():
        print(f"❌ Error: {toolbar_script} not found!")
        sys.exit(1)
    
    print("="*60)
    print("🔧 ALIVE Data - Development Mode (Hot Reload)")
    print("="*60)
    print("📝 Running toolbar.py directly (no EXE rebuild needed)")
    ...
    # Set up file watcher
    event_handler = ReloadHandler(toolbar_script)
    observer = Observer()

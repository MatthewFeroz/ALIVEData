...
export const summarizeText = action({
  ...
  handler: async (ctx, args) => {
    ...
    const prompt = `You are a technical documentation writer. Convert the following OCR text from a screenshot into a clear, well-formatted README-style markdown document.
...
Create a README.md style document that:
1. Has a clear title describing what was captured
2. Includes a brief description/summary
3. Lists any commands or code snippets in proper code blocks
4. Explains what the user was doing
5. Notes any important details, errors, or configurations
6. Uses proper markdown formatting (headers, code blocks, lists)
